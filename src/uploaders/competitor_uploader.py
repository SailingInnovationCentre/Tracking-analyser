...
class CompetitorUploader:    
    ...
    def upload_entries(self, json_path, conn, cursor) : 
        with open(json_path) as json_file_object : 
            ...
            for record in competitors_lst : 
                list_to_upload.append((record['id'], record['name'], record['nationality'], regatta_id, record['boat']['sailId']))

            query = "INSERT INTO powertracks.competitors(comp_id, comp_name, nationality, regatta_id, sail_id) VALUES (?,?,?,?,?)"
    ...
    def upload_avg_json(self, json_path, conn, cursor) : 
        with open(json_path) as json_file_object : 
            ...
            query = "UPDATE powertracks.competitors SET speed_kts_avg = ? WHERE comp_name = ? AND regatta_id = ?"
            cursor.executemany(query, list_to_upload)
            cursor.commit()
    ...
    def upload_avg_legtype_json(self, json_path, conn, cursor) :
        with open(json_path) as json_file_object : 
            ...
            if len(list_downwind_to_upload) > 0 : 
                query = "UPDATE powertracks.competitors SET speed_kts_downwind_avg = ? WHERE comp_name = ? AND regatta_id = ?"
                cursor.executemany(query, list_downwind_to_upload)
                cursor.commit()

            if len(list_upwind_to_upload) > 0 : 
                query = "UPDATE powertracks.competitors SET speed_kts_upwind_avg = ? WHERE comp_name = ? AND regatta_id = ?"
                cursor.executemany(query, list_upwind_to_upload)
                cursor.commit()

            if len(list_reaching_to_upload) > 0 : 
                query = "UPDATE powertracks.competitors SET speed_kts_reaching_avg = ? WHERE comp_name = ? AND regatta_id = ?"
    ...
    def upload_dist_json(self, json_path, conn, cursor) : 
        with open(json_path) as json_file_object : 
            ...
            query = "UPDATE powertracks.competitors SET distance_sum = ? WHERE comp_name = ? AND regatta_id = ?"
            cursor.executemany(query, list_to_upload)
            cursor.commit()
    ...
    def upload_dist_legtype_json(self, json_path, conn, cursor) :
        with open(json_path) as json_file_object : 
            ...
            if len(list_downwind_to_upload) > 0 : 
                query = "UPDATE powertracks.competitors SET distance_sum_downwind = ? WHERE comp_name = ? AND regatta_id = ?"
                cursor.executemany(query, list_downwind_to_upload)
                cursor.commit()

            if len(list_upwind_to_upload) > 0 : 
                query = "UPDATE powertracks.competitors SET distance_sum_upwind = ? WHERE comp_name = ? AND regatta_id = ?"
                cursor.executemany(query, list_upwind_to_upload)
                cursor.commit()

            if len(list_reaching_to_upload) > 0 : 
                query = "UPDATE powertracks.competitors SET distance_sum_reaching = ? WHERE comp_name = ? AND regatta_id = ?"
                cursor.executemany(query, list_reaching_to_upload)
                cursor.commit()
    ...
    def upload_maneuvers_json(self, json_path, conn, cursor) : 
        with open(json_path) as json_file_object : 
            ...
            query = "UPDATE powertracks.competitors SET sum_nr_maneuvers = ? WHERE comp_name = ? AND regatta_id = ?"
            cursor.executemany(query, list_to_upload)
            cursor.commit() 