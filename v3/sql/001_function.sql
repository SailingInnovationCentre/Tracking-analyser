...
CREATE FUNCTION powertracks3.FindOrthPosOnLine(@l1x float, @l1y float, @l2x float, @l2y float, @px float, @py float)
  RETURNS DECIMAL(9,3)
  AS
  BEGIN
    DECLARE @r as float; 
...
CREATE FUNCTION powertracks3.GetSmallest
(
...
CREATE FUNCTION powertracks3.FindAngle(
    @deg1 int,  @deg2 int
)
RETURNS int
AS
